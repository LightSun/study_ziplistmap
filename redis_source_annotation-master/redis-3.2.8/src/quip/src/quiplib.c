...
#define _LOG(fmt, ...) fprintf(stdout, fmt, __VA_ARGS__)

bool quip_verbose = false;
const char* quip_prog_name = "quip";
const char* quip_in_fname = "unknown_file";
...
quip_out_t* quip_out_open(
              ...
              const seqmap_t*   ref)
{
    _LOG("quip_out_open: format,ops = %d, %d\n", fmt, opts);
...
quip_in_t* quip_in_open_file(
              ...
              const seqmap_t* ref)
{
    _LOG("quip_in_open_file: filter,format,ops = %d, %d, %d\n", filter, format, opts);
...
FILE* _quip_open_fin(const char* fn)
{
    quip_in_fname = fn;
    FILE* f = fopen(fn, "rb");
...
//    FILE* f = fdopen(fd, "rb");
    ...
    if (f == NULL) {
        //quip_error("Error opening file: %s.", fn);
        //close(fd);
        return NULL;
    }
